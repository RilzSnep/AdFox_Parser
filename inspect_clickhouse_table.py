...
def fetch_all_rows(client: Client, full_table_name: str) -> tuple[list[str], list[tuple]]:
...
def save_outputs(output_dir: Path, table_name: str, df: pd.DataFrame) -> tuple[Path, Path]:
    ...
    csv_path = output_dir / f"{table_name}_full_export_{timestamp}.csv"
    xlsx_path = output_dir / f"{table_name}_full_export_{timestamp}.xlsx"
...
def main() -> None:
    ...
    try:
        ...
        columns, rows = fetch_all_rows(client, full_table_name)
        ...
        csv_path, xlsx_path = save_outputs(
            output_dir=config["output_dir"] / "clickhouse_exports",
